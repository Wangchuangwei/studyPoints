...
def main(args):
    ...
    # logger.info("进入到main1函数")
    # logger.info(args.dataset_root)

    model = None
    tokenizer = None
    if args.do_pre_train: 
        logger.info("do_pre_train")
...
if __name__ == '__main__':
    ...
    # Root for saving models
    main_args.preds_root = os.path.join(main_args.output_root, 'preds')
    ...
    for d in [main_args.checkpoint_root, main_args.model_root, main_args.vocab_root, main_args.tensor_board_root,
              main_args.dataset_save_dir, main_args.vocab_save_dir, main_args.pre_train_output_root, main_args.preds_root]:
    ...
    #handler,log file
    file = logging.FileHandler(os.path.join(main_args.output_root, 'study_2_ast_test.log'))
