...
class Vocab(object):
    ...
    ADD_TOKEN = '[ADD]'  # add line token
    KEEP_TOKEN = '[KEEP]'  # keep line token
    DEL_TOKEN = '[DEL]'  # del line token

    # default special symbols, if need additional symbols, use init parameter 'additional_special_symbols'
    START_VOCAB = [PAD_TOKEN, SOS_TOKEN, EOS_TOKEN, UNK_TOKEN, MSK_TOKEN, SEP_TOKEN, KEEP_TOKEN, ADD_TOKEN, DEL_TOKEN]
    ...
    def __init__(
        ...
        # train tokenizer
        logger.info(f"{type(datasets[0])}")
        logger.info(f"{self.__special_symbols}")
    ...
    def get_add_index(self):
        return self.tokenizer.token_to_id(Vocab.ADD_TOKEN)

    def get_del_index(self):
        return self.tokenizer.token_to_id(Vocab.DEL_TOKEN)
    
    def get_keep_index(self):
        return self.tokenizer.token_to_id(Vocab.KEEP_TOKEN)
