...
class Seq2Seq_pretrain(nn.Module):       
    ...
    def forward(self, source_ids, task, target_ids=None, labels=None, line_level_code=None, token_level_code=None, old_ast_ids=None, new_ast_ids=None, old_ast_maps=None, new_ast_maps=None):   
        ...
        # for msg level action
        msg_mask = target_ids.ne(1)[:,None,:]*target_ids.ne(1)[:,:,None]
        msg_encoder_outputs = self.encoder_msg(target_ids,attention_mask=msg_mask,use_cache=True)  
        outputs_msg = msg_encoder_outputs[0]
        outputs_msg = torch.cat([outputs_msg, torch.zeros(self.batch_size, self.source_length - self.max_length, self.config.hidden_size, device=outputs_msg.device )], 1)
        ...
        final_output = torch.cat((outputs[:,:self.source_length,:]+ ast_h_final + h_final + outputs_msg, outputs[:,self.source_length:,:]), 1)
