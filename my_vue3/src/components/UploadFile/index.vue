<template>
    <el-button @click="handleView1">文件列表</el-button>
      ...
      <el-table :data="tableData" stripe class="table-demo">
          ...
          <el-table-column prop="percentage" label="操作">
              <template #default="scope">
                ...
                <el-button type="primary" v-if="scope.row.status=='uploading'" @click="handlePause(scope.row)">暂停</el-button>
                <el-button type="primary" v-else-if="scope.row.status=='fail'" @click="handleResume(scope.row)">继续</el-button>   
...
import {createFileChunk, calculateHash} from './modules/index.ts'
 ...
 const uploadChunks = async (type: string, data: uploadData[], tabDataIndex: number, filename: string, filehash: string, size: number, basePercentage: number) => {
    ...
    try {
        ...
        if (success.length === data.length){
            mergeRequest({type: type, size: SIZE, fileHash: filehash, filename: filename}) 
            tableData[tabDataIndex].percentage = 100
            tableData[tabDataIndex].status = 'success'
            console.log('全部上传')
        } else {
...
const handleView = async (file: UploadFile) => {
    console.log('预览：', file)
    const fileExtension = file.name.split('.').pop()
    if (fileExtension == 'xlsx') {
        fileViewerRef.value?.viewXlsx(file.raw)
    } else if (fileExtension == 'docx') {
        fileViewerRef.value?.viewDocx(file.raw)
    } else if (fileExtension == 'pdf') {
        fileViewerRef.value?.viewPdf(file.raw)
...
const handleView1 = async () => {
    console.log('预览1')
    const result = await getFileList('before')
